                        ...
                        @foreach ($data as $key => $value)
                            @if ($key > 0)
                                ...
                                <div class="blog-list"
                                    ...
                                    {{-- <form action="{{ route('detailpage', $value->slug) }}" method="POST">
                                        @csrf
                                        <input type="text" name="type" value="blog" style="display: none">
                                        <button title="{{ @$value->title }}"
                                            style="display:block;text-decoration:none;font-size:20px;line-height: 27px;color: #212121;font-weight: 600;border: none;background: transparent;font-weight: bold;">
                                            <img src="{{ asset(@$value->featured_image) }}" alt="{{ @$value->title }}"
                                                style="width:100%;height:200px;object-fit:cover;">
                                            <span style="display:block;margin-top:10px;"> {!! @$value->title !!}</span>
                                        </button>
                                    </form> --}}
                                    {{-- {{dd(env('APP_URL').'/'.@$value->slug)}} --}}
