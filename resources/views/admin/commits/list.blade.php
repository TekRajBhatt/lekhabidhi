        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Committed To Change List</h3>
                ...
                <div class="card-header">
                    <div class="row">
                        <div class="btn-group col-lg-1">
                            <a href="{{ route('commit.index') }}" class="btn btn-primary btn-flat btn-sm">
                                <i class="fas fa-sync-alt fa-sm"></i> Refresh
                            </a>
                        </div>
                        <div class="col-lg-7">
                            <form action="{{route('commit.index')}}" class="" method="get">
                                <div class="row">
                        ...
                        <div class="col-lg-4">
                            <div class="card-tools float-right">
                                @can('commit-create')
                                <a href="{{ route('commit.create') }}" class="btn btn-success btn-sm btn-flat mr-2">
                                    <i class="fa fa-plus"></i> Add New Commit</a>
                                @endcan
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            ...
                            <td>{{@$value->view_count}}</td>
                              ...
                              <td>
                                <div class="btn-group">
                                  @can('commit-edit')
                                  <a href="{{route('commit.edit',$value->id)}}" title="Edit commit" class="btn btn-success btn-sm btn-flat"><i class="fas fa-edit"></i></a>
                                  @endcan
                                  @can('commit-delete')
                                  {{Form::open(['method' => 'DELETE','route' => ['commit.destroy', $value->id],'style'=>'display:inline','onsubmit'=>'return confirm("Are you sure you want to delete this commit?")']) }}
                                  {{Form::button('<i class="fas fa-trash-alt"></i>',['class'=>'btn btn-danger btn-sm btn-flat','type'=>'submit','title'=>'Delete commit '])}}
                                  {{ Form::close() }}
                                  @endcan
                              </div>
                              </td>
