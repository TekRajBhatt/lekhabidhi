                ...
                <div class="card-header">
                    <div class="row">
                        <div class="btn-group col-lg-1">
                            <a href="{{ route('step.index') }}" class="btn btn-primary btn-flat btn-sm">
                                <i class="fas fa-sync-alt fa-sm"></i> Refresh
                            </a>
                        </div>
                        <div class="col-lg-7">
                            <form action="{{route('step.index')}}" class="" method="get">
                                <div class="row">
                        ...
                        <div class="col-lg-4">
                            <div class="card-tools float-right">
                                @can('step-create')
                                <a href="{{ route('step.create') }}" class="btn btn-success btn-sm btn-flat mr-2">
                                    <i class="fa fa-plus"></i> Add New Step</a>
                                @endcan
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody>
                            ...
                            <td>
                              ...
                              <td>
                                <div class="btn-group">
                                  @can('step-edit')
                                  <a href="{{route('step.edit',$value->id)}}" title="Edit step" class="btn btn-success btn-sm btn-flat"><i class="fas fa-edit"></i></a>
                                  @endcan
                                  @can('step-delete')
                                  {{Form::open(['method' => 'DELETE','route' => ['step.destroy', $value->id],'style'=>'display:inline','onsubmit'=>'return confirm("Are you sure you want to delete this step?")']) }}
                                  {{Form::button('<i class="fas fa-trash-alt"></i>',['class'=>'btn btn-danger btn-sm btn-flat','type'=>'submit','title'=>'Delete step '])}}
                                  {{ Form::close() }}
                                  @endcan
                              </div>
                              </td>
